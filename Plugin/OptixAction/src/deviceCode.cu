...
OPTIX_RAYGEN_PROGRAM(simpleRayGen)()
{
    ...
    const int eyeIdx = optixLaunchParams.eyeIndex;
    const NativeCameraData& camData = optixLaunchParams.nativeCameraData;
    ...
    owl::Ray ray;
    ray.origin = camData.cameraPositionWorld;
    ray.direction = normalize(camData.cameraToUpperLeftScreen + (float)pixelID.x *  camData.pxStepU + (float)pixelID.y * camData.pxStepV);
    if(pixelID == self.fbSize / 2)
    {
        vec3f worldStartPos = camData.cameraPositionWorld;
    ...
    surf2Dwrite(owl::make_rgba(prd.color), self.surf[eyeIdx], sizeof(uint32_t) * pixelID.x,
                pixelID.y);
}

OPTIX_CLOSEST_HIT_PROGRAM(TriangleMesh)()
{
    PerRayData1& prd = owl::getPRD<PerRayData1>();
    ...
    vec3f orig = optixGetWorldRayOrigin();

    vec3f worldPos = ((rayDir * optixGetRayTmax()) + orig) * 0.5f + vec3f(0.5f);

    prd.color = { worldPos, 1.0f };
}
