...
SourceVolumeRendererTest::SourceVolumeRendererTest(Texture* texture, std::vector<SourceRegion>& sourceRegions, std::vector<float>& sourceRegionDataBuffer) : _texture(texture)
{
    _context = owlContextCreate(nullptr, 1);
    _module = owlModuleCreate(_context, deviceCode_ptx);

    OWLBuffer owlSourcesDataBuffer = owlDeviceBufferCreate(_context, OWL_USER_TYPE(SourceRegion), sourceRegions.size(), sourceRegions.data());
    OWLBuffer owlDataBuffer = owlDeviceBufferCreate(_context, OWL_FLOAT, sourceRegionDataBuffer.size(), sourceRegionDataBuffer.data());
    const owl2f gridDataMinMax =
    {
      *std::min_element(sourceRegionDataBuffer.begin(), sourceRegionDataBuffer.end()),
      *std::max_element(sourceRegionDataBuffer.begin(), sourceRegionDataBuffer.end())
    };
...
SourceVolumeRendererTest::~SourceVolumeRendererTest()
{
    if (_context != nullptr)
    {
...
void SourceVolumeRendererTest::setTexture(Texture* texture)
{
...
void SourceVolumeRendererTest::render(void* data)
{    
